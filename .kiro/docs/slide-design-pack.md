...
- When using two-col, vertically centre both columns with `align-items: center`.
...
Always use `align-items: center` on `.two-col` so left and right columns are vertically centred against each other.
