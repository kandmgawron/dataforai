...
def get_products(qs):
    ...
    sql = "SELECT sku, name, brand, category_l1, price_gbp, sale_price_gbp, short_description, total_stock FROM products WHERE is_active = true"
    params = []
    ...
    products = [row_to_dict(r, ["sku", "name", "brand", "category_l1", "price_gbp", "sale_price_gbp", "short_description", "total_stock"]) for r in rows]
...
def get_policies():
    """List policy documents from S3."""
    try:
        resp = s3.list_objects_v2(Bucket=POLICIES_BUCKET, Prefix="policies/")
        docs = []
        for obj in resp.get("Contents", []):
            key = obj["Key"]
            doc_id = key.split("/")[-1].replace(".txt", "")
            if doc_id:
                docs.append({"doc_id": doc_id, "title": doc_id, "key": key})
...
def get_reviews(sku):
    sql = """SELECT review_id, customer_id, rating, title, body, created_at
             FROM reviews WHERE sku = :sku ORDER BY created_at DESC LIMIT 20"""
    params = [{"name": "sku", "value": {"stringValue": sku}}]
    rows = execute_sql(sql, params)
...
def execute_sql(sql, params=None):
    """Execute SQL via RDS Data API, return records."""
    kwargs = {
